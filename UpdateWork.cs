...
namespace AutoUpdate
{
    public class UpdateWork
    {
        ...
        string bakPath = Path.Combine(Environment.GetEnvironmentVariable("TEMP"), @"AutoUpdate\bak\");
        ...
        /// 初始化配置目录信息
        /// </summary>
        public UpdateWork( string localAddress, string isClickUpdate)
        {
            ...
            localInfo.LoadXml();
            if (string.IsNullOrEmpty(localInfo.AppName))
            {
                LogTool.AddLog("更新程序：Local.xml配置文件的<AppName>值为升级程序名称，不允许为空！");
                throw new Exception("Local.xml配置文件的<AppName>值为升级程序名称，不允许为空！");
            }
            programName = localInfo.AppName;
            UpdateVerList = GetServer(localInfo.ServerUpdateUrl, programName);
        ...
        /// <returns></returns>
        private static List<RemoteInfo> GetServer(string url, string programName)
        {
            ...
            var listNodes = root.SelectNodes("/ServerUpdate/Item[@AppName=\"" + programName + "\"]");
            ...
            using (WebClient web = new WebClient())
            {
                foreach (var item in UpdateVerList)
                {
                    try
                    {
                        ...
                        if (OnUpdateProgess != null) OnUpdateProgess.Invoke(60 / UpdateVerList.Count);
                    }
                    catch (Exception ex)
                    {
        ...
        private UpdateWork Bak()
        {
            try
            {
                ...
                if (OnUpdateProgess != null) OnUpdateProgess.Invoke(20);
                return this;
            }
            catch (Exception EX)
            {
            ...
            foreach (var item in UpdateVerList)
            {
                try
                {
                    ...
                    using (ZipFile zip = new ZipFile(path))
                    {
                        ...
                        if (item.UpdateMode == "Installation")
                        {
                            zip.ExtractAll(tempPath, ExtractExistingFileAction.OverwriteSilently);
                            Process proc = Process.Start(Path.Combine(tempPath, item.ApplicationUpdateStart));
                            if (proc != null)
                            {
                                proc.WaitForExit();
                            }
                ...
                finally
                {
                    ...
                    if (item.UpdateMode == "Installation")
                    {
                        DelTempFile(item.ApplicationUpdateStart);
                    }
                    LogTool.AddLog("更新程序：临时文件删除完成" + item.ReleaseVersion);
                }
            }
            if (OnUpdateProgess != null) OnUpdateProgess.Invoke(98);
            return this;
        }
        ...
        private UpdateWork Start()
        {
            ...
            if (OnUpdateProgess != null) OnUpdateProgess.Invoke(100);
            return this;
        }
